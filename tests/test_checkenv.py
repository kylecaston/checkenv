...
def test_main_import_invalid_json_raise_exception_no_output(init_env, capsys):
    with pytest.raises(ValidationError) as exc:
        check(
            filename=os.path.join(dir_path, "fixtures/invalid.json"),
...
def test_main_import_raise_exception_on_error_details(init_env):
    try:
        ...
        assert set(cee.optional) == set(
            ["VALUE3_NOT_SET_NOT_REQUIRED", "VALUE2_NOT_SET_WITH_DEFAULT"]
        )
...
def test_mandatory_value_not_set_valid2(init_env, monkeypatch):
    ...
    instance.load_spec_file()
    instance.apply_spec()
    assert instance.missing == ["VALUE1_NOT_SET"]
    assert {"VALUE3_NOT_SET_NOT_REQUIRED", "VALUE2_NOT_SET_WITH_DEFAULT"} == set(instance.optional)
